...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
                      rect=False, rank=-1, workers=8, image_weights=False, quad=False, prefix='', shuffle=False,copy_paste_samll=False):
    if rect and shuffle:
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(path, imgsz, batch_size,
                                      ...
                                      prefix=prefix,
                                      copy_paste_samll=copy_paste_samll
                                      )
...
class LoadImages:
    ...
    def __next__(self):
        ...
        # Padded resize。#tag:这一步很关键，letterbox将输入不规则的的图像(1280*800比如)，变成640*640的。计算最小的填充
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',copy_paste_samll=False):
        self.img_size = img_size
        ...
        self.copy_paste_samll=copy_paste_samll
    ...
    def __getitem__(self, index):
        ...
        #tag:加入copy paste增强
        if self.copy_paste_samll:
            img,labels=copy_paste_small_fun(img,labels,
                                        thresh=hyp['thresh'], 
                                        prob=hyp['prob'], 
                                        copy_times=hyp['copy_times'], 
                                        epochs=hyp['epochs'],
                                        all_objects=hyp['all_objects'], 
                                        one_object=hyp['one_object'])
...
def donot_overlap( new_l, labels):
    for l in labels:
        if compute_overlap(new_l, l): return False
    return True

def create_copy_label( h, w, l, labels,epochs):
    l = l.astype(np.int)
...
def add_patch_in_img( new_label, l, image):
    l = l.astype(np.int)
    image[new_label[2]:new_label[4], new_label[1]:new_label[3], :] = image[l[2]:l[4], l[1]:l[3], :]
    return image

def copy_paste_small_fun(image,labels ,thresh=32*32, prob=0.5, copy_times=3, epochs=30, all_objects=False, one_object=False):
        """
        ...
        for i in range(labels.shape[0]):
            ...
            if str(label[0]) == '1.0':     #tag：如果是cls类别是0就扩增
                label_h, label_w = label[4] - label[2], label[3] - label[1]
                if issmallobject(label_h, label_w,thresh):
        ...
        for idx in range(copy_object_num):
            l = select_label[idx]
            l_h, l_w = l[4] - l[2], l[3] - l[1]
            if issmallobject(l_h, l_w,thresh) is False: continue

            for i in range(copy_times):
                new_label = create_copy_label(h, w, l, labels,epochs)
                if new_label is not None:
                    image = add_patch_in_img(new_label, l, image)
